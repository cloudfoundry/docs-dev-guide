...
When you redeploy an application, cf does _not_ refer to `manifest.yml` for deployment settings. Instead, cf deploys the application with the currently active deployment settings. If you want the settings in the manifest to be applied on subsequent pushes, use the `--reset` option. For more information, see [cf push and the Manifest](../deploy-apps/manifest.html#push-and-manifest) on the [Application Manifests](../deploy-apps/manifest.html) page.

When you deploy an application while it is running, cf stops all instances of that application and then deploys. Users who try to run the application get a "404 not found" message while cf push is running. Stopping all instances is necessary to prevent two versions of your code running at the same time. That would be especially bad if you were deploying an update that involved a database schema migration, because instances running the old code would not work and users could lose data.

Updating Cloud Foundry works differently than `cf push`. When you update Cloud Foundry while multiple instances of an application are running, Cloud Foundry stops one application instance, completes the update, then restarts the instance. Cloud Foundry repeats this process with each instance in turn. This ensures that the application remains available during the Cloud Foundry update, assuming that there are two or more application instances to begin with. To take advantage of this, it's a good idea to always deploy a minimum of two instances of your applications.

cf uploads all application files except version control files with file extensions `.svn`, `.git`, and `.darcs`. If you wish to exclude other files from upload, specify them in a `.cfignore` file in the directory where you run the push command. `.cfignore` behaves similarly to `.gitignore`. For more information, see [Exclude Unnecessary Files with cfignore](../deploy-apps/prepare-to-deploy.html#exclude) on [Prepare to Deploy an Application](../deploy-apps/prepare-to-deploy.html).
